...
// ── Stubs (formal env, auth disabled) ─────────────────────────────

export async function getAccessToken(): Promise<string> {
  return "";
}

export async function getUserByCode(_code: string): Promise<WecomUser> {
  return { userId: "", name: "" };
}

export function buildOAuthUrl(_redirectUri: string, _state = "/"): string {
  return "/";
}
