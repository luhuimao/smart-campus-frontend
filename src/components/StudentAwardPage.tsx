...
export function StudentAwardPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const handleSubmit = () => { setSubmitted(true); if ([semester, grade, className, studentName, stuId, awardLevel, awardRank, competitionName, awardDate, awardUnit, groupType, teacher].find((f) => !f)) window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  return (<div className="flex flex-col h-full overflow-hidden" style={{ color: "#1d1d1f" }}>
    ...
    <div className="flex-1 overflow-y-auto bg-[#f5f5f7] pb-24"><div className="max-w-5xl mx-auto mt-4 md:mt-10 px-3 md:px-6"><div className="rounded-2xl md:rounded-[28px] shadow-sm border border-gray-100 bg-white px-6 md:px-10 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
        <Field label="年级" required><SelectField value={grade} onChange={setGrade} options={gradeOptions} />{submitted && !grade && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        <Field label="班级名称" required><Input value={className} onChange={setClassName} />{submitted && !className && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        ...
        <Field label="学生学号" required><Input value={stuId} onChange={setStuId} placeholder="请输入学号" />{submitted && !stuId && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        <Field label="获奖级别" required><SelectField value={awardLevel} onChange={setAwardLevel} options={["国家级","省级","市级","县级","校级"]} />{submitted && !awardLevel && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        <Field label="获奖等级" required><Input value={awardRank} onChange={setAwardRank} placeholder="例如：一等奖/第一名" />{submitted && !awardRank && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        <Field label="参与比赛名称" required><Input value={competitionName} onChange={setCompetitionName} placeholder="请输入比赛名称" />{submitted && !competitionName && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        <Field label="获奖时间" required><DateTimePicker value={awardDate} onChange={setAwardDate} />{submitted && !awardDate && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        <Field label="颁发单位" required><Input value={awardUnit} onChange={setAwardUnit} placeholder="以证书公章为准" />{submitted && !awardUnit && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        <Field label="个人/团体获奖" required><SelectField value={groupType} onChange={setGroupType} options={["个人","团体"]} />{submitted && !groupType && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
      </div>
      <SectionHeader title="指导老师" />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
        <Field label="指导老师" required><MiniStaffPicker value={teacher} onChange={setTeacher} />{submitted && !teacher && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
      </div>
    </div></div></div>
