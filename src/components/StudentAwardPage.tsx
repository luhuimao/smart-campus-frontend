...
import { useStudentAward, useTermInfo, useGradeInfo, useDepartmentMembers, type StudentAwardRecord, type StudentInfoRecord } from "@/hooks/use-research-dashboard";
...
export function StudentAwardPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const [previewRecord, setPreviewRecord] = useState<StudentAwardRecord | null>(null);
  ...
  const perms = useFormPermissions(JDY_CONFIG.STUDENT_AWARD_INFO.entry_id);
            ...
            <div className="glass rounded-[32px] overflow-hidden flex flex-col shadow-sm" style={{ minHeight: 500 }}>
              ...
              {isPending ? (
                ...
                <DataTable columns={COLUMNS} rows={sorted} minWidth={1400}
                  ...
                  onRowClick={r => setPreviewRecord(r as StudentAwardRecord)} />
              )}
        ...
        <div className="fixed top-0 right-0 h-full z-50 flex flex-col shadow-2xl" style={{ width: 440, maxWidth: "100vw", background: "#fff", transform: r ? "translateX(0)" : "translateX(100%)", transition: "transform 0.3s" }}>
          {r && (<>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6"><section><p className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3">基本信息</p><div className="grid grid-cols-2 gap-x-6 gap-y-4">
              {[{ label: "学期", value: r.学期 },{ label: "年级", value: r.年级 },{ label: "班级", value: r.班级名称 },{ label: "学生姓名", value: r.学生姓名 },{ label: "获奖级别", value: r.获奖级别 },{ label: "获奖等级", value: r.获奖等级 },{ label: "比赛名称", value: r.参与比赛名称 },{ label: "获奖时间", value: r.获奖时间?.slice(0,10) },{ label: "颁发单位", value: r.颁发单位 },{ label: "个人/团体", value: r.个人团体获奖 },{ label: "指导老师", value: r.指导老师 },{ label: "提交人", value: r.提交人 }].map(({ label, value }) => (<div key={label}><p className="text-sm text-gray-400 mb-0.5">{label}</p><p className="text-base font-medium text-gray-800">{value||"—"}</p></div>))}
            </div></section></div>
