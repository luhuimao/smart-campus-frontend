...
import { Bell, ChevronDown, ChevronRight, Plus, Image, Calendar, ClipboardList, X, CircleDot, Menu } from "lucide-react";
...
type WorksRow = { id: number };

export function WorksPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  const [hasWorks, setHasWorks] = useState<"yes" | "no" | null>(null);
  const [rows, setRows] = useState<WorksRow[]>([{ id: 1 }]);
              ...
              <>
                ...
                <div className="p-8 bg-white border-t border-gray-50">
                  ...
                  <div className="w-full border border-gray-100 rounded-xl overflow-hidden text-sm">
                    {/* Header */}
                    <div className="grid bg-gray-50 border-b border-gray-100" style={{ gridTemplateColumns: "48px 2fr 1.2fr 1fr 1fr" }}>
                    ...
                    {/* Rows */}
                    {rows.map((row, index) => (
                      <div key={row.id} className="grid border-b border-gray-100 last:border-0" style={{ gridTemplateColumns: "48px 2fr 1.2fr 1fr 1fr" }}>
