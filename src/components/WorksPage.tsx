...
import { JDY_CONFIG, TEACHER_MONOGRAPHS_WIDGET_IDS, jdyCreate, jdyUploadFiles } from "@/lib/jdy-api";
...
export function WorksPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const addRow = useCallback(() => {
    setRows((prev) => [...prev, { id: nextId, name: "", role: "", year: "", images: [] }]);
    setNextId((n) => n + 1);
  }, [nextId]);
  ...
  const anyMissing = useMemo(
    () => rows.some((r) => !r.name || !r.role || !r.year || r.images.length === 0),
    [rows],
  );
  ...
  const H = TEACHER_MONOGRAPHS_WIDGET_IDS;

  const buildData = (allKeys: string[]) => {
    let ki = 0;
    const subRecords = rows.map(row => ({
      [H["著作.编号"]]: { value: "" },
      [H["著作.著作名称"]]: { value: row.name },
      [H["著作.角色类型"]]: { value: row.role },
      [H["著作.出版年份"]]: { value: row.year },
      [H["著作.图片证明材料"]]: { value: row.images.map(() => allKeys[ki++]) },
    }));

    const data: Record<string, { value: unknown }> = {
      [H.教师姓名]: { value: toMember(teacherName) },
      [H.身份证号码]: { value: idCard },
      [H.有无著作]: { value: hasWorks === "yes" ? "有" : "暂无" },
    ...
    try {
      ...
      if (allFiles.length > 0) {
        ...
        const { keys } = await jdyUploadFiles(
          allFiles,
          JDY_CONFIG.TEACHER_MONOGRAPHS_INFO.app_id,
          JDY_CONFIG.TEACHER_MONOGRAPHS_INFO.entry_id,
          transaction_id,
        );
        allKeys = keys;
      }
      const res = await jdyCreate({
        app_id: JDY_CONFIG.TEACHER_MONOGRAPHS_INFO.app_id,
        entry_id: JDY_CONFIG.TEACHER_MONOGRAPHS_INFO.entry_id,
  ...
  const handleClearForm = () => {
    ...
    setRows([{ id: 1, name: "", role: "", year: "", images: [] }]);
    setNextId(2);
    setSubmitted(false);
    localStorage.removeItem("works-draft");
  };
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("works-draft");
      if (!raw) return;
      ...
      if (d.hasWorks) setHasWorks(d.hasWorks);
      if (d.rows && d.rows.length > 0) {
        setRows(d.rows.map((r: WorksRow) => ({ ...r, images: [] })));
        setNextId(Math.max(...d.rows.map((r: WorksRow) => r.id)) + 1);
      }
    } catch {}
  }, []);
  ...
  return (
    ...
    >
      <PageHeader
        centered
        breadcrumbs={[{ label: "教师基础档案" }, { label: "著作", active: true }]}
        ...
        <main className="max-w-6xl mx-auto pt-4 md:pt-6 px-3 md:px-6 pb-24">
          ...
          <div className="mb-8 text-center">
            <div className="inline-flex flex-col items-center">
              <h2 className="text-xl font-bold tracking-tight" style={{ color: "#111827" }}>著作</h2>
          ...
          <div className="rounded-[28px] shadow-sm border border-gray-100 bg-white">
            ...
            <div className="p-8 bg-white">
              <Field label="有无著作" required>
                <div className="flex items-center gap-10 py-2">
                  {[{ v: "yes" as const, l: "有" }, { v: "no" as const, l: "暂无" }].map(({ v, l }) => (
                    <label key={v} className="flex items-center gap-2.5 cursor-pointer" onClick={() => setHasWorks(v)}>
                      <div
                        className="w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all shrink-0 bg-white"
                        style={{ borderColor: hasWorks === v ? teal : "#d1d5db" }}
                      >
                        {hasWorks === v && <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: teal }} />}
                      </div>
              ...
              <>
                ...
                <div className="p-8 bg-white border-t border-gray-50">
                  <p className="text-base font-semibold mb-5">
                    <span style={{ color: "#ff4d4f", marginRight: 4 }}>*</span>著作
                  </p>

                  <div className="w-full border border-gray-100 rounded-xl overflow-hidden text-sm"
                    style={{ display: "grid", gridTemplateColumns: "48px 2fr 1.2fr 1fr 160px 52px" }}>
                    {["", "著作名称", "角色类型", "出版年份", "图片证明材料", ""].map((h, i) => (
                      ...
                      <>
                        ...
                        <div key={`yr-${row.id}`} className="px-2 py-2 bg-white border-b border-r border-gray-100">
                          <input
                            ...
                            style={{ color: row.year ? "#374151" : "#9ca3af", borderColor: submitted && !row.year ? "#ff4d4f" : undefined }}
                            onFocus={(e) => Object.assign(e.currentTarget.style, focusStyle)}
                            onBlur={(e) => Object.assign(e.currentTarget.style, blurStyle)}
                          />
                        </div>
                  ...
                  {submitted && hasWorks === "yes" && anyMissing && (
                    <div className="mt-3 space-y-0.5">
                      {rows.some((r) => !r.name) && <p className="text-xs" style={{ color: "#ff4d4f" }}>著作名称为必填项</p>}
                      {rows.some((r) => !r.role) && <p className="text-xs" style={{ color: "#ff4d4f" }}>角色类型为必填项</p>}
                      {rows.some((r) => !r.year) && <p className="text-xs" style={{ color: "#ff4d4f" }}>出版年份为必填项</p>}
