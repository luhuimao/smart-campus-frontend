...
import { Plus, Image as ImageIcon, ClipboardList, Menu, Trash2 } from "lucide-react";
...
function Field({ label, required, hint, children }: { label: string; required?: boolean; hint?: string; children: React.ReactNode }) {
  return (<div><label className="block text-base font-semibold mb-2" style={{ color: "#1d1d1f" }}>{required && <span style={{ color: "#ff4d4f", marginRight: 4 }}>*</span>}{label}</label>{hint && <p className="text-[11px] text-gray-400 mb-2">{hint}</p>}{children}</div>);
}
...
type WorksRow = { id: number; name: string; role: string; year: string; image: string | null };

export function WorksPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const [hasWorks, setHasWorks] = useState<"yes" | "no">("yes");
  const [submitted, setSubmitted] = useState(false);

  const [rows, setRows] = useState<WorksRow[]>([{ id: 1, name: "", role: "", year: "", image: null }]);
  const [nextId, setNextId] = useState(2);
  const updateRow = useCallback((id: number, f: string, v: string | null) => setRows((p) => p.map((r) => (r.id === id ? { ...r, [f]: v } : r))), []);
  const addRow = useCallback(() => { setRows((p) => [...p, { id: nextId, name: "", role: "", year: "", image: null }]); setNextId((n) => n + 1); }, [nextId]);
  const removeRow = useCallback((id: number) => setRows((p) => (p.length <= 1 ? p : p.filter((r) => r.id !== id))), []);
  const anyMissing = useMemo(() => rows.some((r) => !r.name || !r.role || !r.year || !r.image), [rows]);
  ...
  const handleSubmit = () => { setSubmitted(true); if ([{ f: teacherName }, { f: idCard }, ...(hasWorks === "yes" ? [{ f: department }, { f: position }, { f: positionType }, { f: partyJob }, { f: phone }, { f: subject }, { f: !anyMissing }] : [])].find((x) => !x.f)) window.scrollTo({ top: 0, behavior: "smooth" }); };

  return (<div className="flex flex-col h-full overflow-hidden" style={{ color: "#1d1d1f" }}>
    <PageHeader centered left={<FlowButton />} breadcrumbs={[{ label: "教师基础档案" }, { label: "著作", active: true }]} onMenuOpen={onMenuOpen} />
    <div className="flex-1 overflow-y-auto bg-[#f5f5f7] pb-24"><main className="max-w-6xl mx-auto mt-4 md:mt-10 px-3 md:px-6">
      <div className="flex items-center justify-center gap-5 mb-12"><div className="h-px w-20" style={{ background: `linear-gradient(to right, transparent, ${teal}, transparent)` }} /><div className="flex items-center gap-3 px-12 py-2 text-white text-base font-semibold tracking-[0.2em]" style={{ backgroundColor: teal, clipPath: "polygon(10% 0, 90% 0, 100% 50%, 90% 100%, 10% 100%, 0 50%)", boxShadow: "0 4px 12px rgba(0,176,149,0.2)" }}><span className="w-2 h-2 bg-white rotate-45 shrink-0 inline-block" />著作<span className="w-2 h-2 bg-white rotate-45 shrink-0 inline-block" /></div><div className="h-px w-20" style={{ background: `linear-gradient(to right, transparent, ${teal}, transparent)` }} /></div>
      <div className="rounded-[28px] overflow-hidden shadow-sm border border-gray-100 bg-white">
        ...
        <div className="p-8 bg-white"><Field label="有无著作" required><div className="flex items-center gap-10 py-2">{[{ v: "yes" as const, l: "有" }, { v: "no" as const, l: "暂无" }].map(({ v, l }) => (<label key={v} className="flex items-center gap-2.5 cursor-pointer" onClick={() => setHasWorks(v)}><div className="w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all shrink-0 bg-white" style={{ borderColor: hasWorks === v ? teal : "#d1d5db" }}>{hasWorks === v && <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: teal }} />}</div><span className="text-base font-medium text-gray-700">{l}</span></label>))}</div></Field></div>
        {hasWorks === "yes" && (<>
          ...
          <div className="p-8 bg-white border-t border-gray-50"><p className="text-base font-semibold mb-5"><span style={{ color: "#ff4d4f", marginRight: 4 }}>*</span>著作</p>
            <div className="w-full border border-gray-100 rounded-xl overflow-hidden text-sm" style={{ display: "grid", gridTemplateColumns: "48px 2fr 1.2fr 1fr 1fr 52px" }}>
              {["", "著作名称", "角色类型", "出版年份", "图片证明材料", ""].map((h, i) => (<div key={i} className="px-3 py-3 bg-gray-50 text-sm font-semibold text-gray-600 border-b border-r border-gray-100 last:border-r-0">{i > 0 && i !== 4 && i !== 5 && <span style={{ color: "#ff4d4f", marginRight: 3 }}>*</span>}{h}</div>))}
              {rows.map((row, idx) => (<>
                ...
                <div key={`nm-${row.id}`} className="px-2 py-2 bg-white border-b border-r border-gray-100"><input type="text" value={row.name} onChange={(e) => updateRow(row.id, "name", e.target.value)} className="table-input" style={submitted && !row.name ? { borderColor: "#ff4d4f" } : undefined} onFocus={(e) => Object.assign(e.currentTarget.style, focusStyle)} onBlur={(e) => Object.assign(e.currentTarget.style, blurStyle)} /></div>
                <div key={`rl-${row.id}`} className="px-2 py-2 bg-white border-b border-r border-gray-100"><select value={row.role} onChange={(e) => updateRow(row.id, "role", e.target.value)} className="table-input appearance-none" style={(!row.role ? { color: "#9ca3af" } : {}) as React.CSSProperties} onFocus={(e) => Object.assign(e.currentTarget.style, focusStyle)} onBlur={(e) => Object.assign(e.currentTarget.style, blurStyle)}><option value=""></option><option>主编</option><option>副主编</option><option>参编</option></select></div>
                <div key={`yr-${row.id}`} className="px-2 py-2 bg-white border-b border-r border-gray-100"><input type="date" value={row.year} onChange={(e) => updateRow(row.id, "year", e.target.value)} className="table-input w-full" style={{ color: row.year ? "#374151" : "#9ca3af", borderColor: submitted && !row.year ? "#ff4d4f" : undefined }} onFocus={(e) => Object.assign(e.currentTarget.style, focusStyle)} onBlur={(e) => Object.assign(e.currentTarget.style, blurStyle)} /></div>
                <div key={`img-${row.id}`} className="px-2 py-2 bg-white border-b border-r border-gray-100"><label className="w-full h-8 border rounded-lg flex items-center justify-center bg-gray-50 text-gray-300 cursor-pointer hover:bg-gray-100 transition-colors overflow-hidden" style={{ borderColor: submitted && !row.image ? "#ff4d4f" : "#e5e7eb" }}>{row.image ? <img src={row.image} alt="" className="w-full h-full object-cover" /> : <ImageIcon className="w-4 h-4" />}<input type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) updateRow(row.id, "image", URL.createObjectURL(f)); }} /></label></div>
            ...
            {submitted && hasWorks === "yes" && anyMissing && (<div className="mt-3 space-y-0.5">{rows.some((r) => !r.name) && <p className="text-xs" style={{ color: "#ff4d4f" }}>著作名称为必填项</p>}{rows.some((r) => !r.role) && <p className="text-xs" style={{ color: "#ff4d4f" }}>角色类型为必填项</p>}{rows.some((r) => !r.year) && <p className="text-xs" style={{ color: "#ff4d4f" }}>出版年份为必填项</p>}{rows.some((r) => !r.image) && <p className="text-xs" style={{ color: "#ff4d4f" }}>图片证明材料为必填项</p>}</div>)}
