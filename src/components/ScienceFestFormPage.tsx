...
import { X, Search, Upload, Menu, ChevronDown, Trash2, Clock } from "lucide-react";
import * as XLSX from "xlsx";
import { useTeachingResearchGroups, useDepartmentMembers, useScienceFestDashboard, type ScienceFestRecord } from "@/hooks/use-research-dashboard";
    ...
    <div className="flex-1 overflow-y-auto bg-[#f5f5f7]">
      ...
      {(showForm || editRecord) ? (<main className="max-w-6xl mx-auto px-3 md:px-6 pb-24">
        ...
        <div className="rounded-[28px] overflow-hidden shadow-sm border border-gray-100 bg-white">
          ...
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 bg-white"><Field label="活动负责教师" required><DeptStaffPicker staffList={deptMembers} value={teacher} onChange={(v) => setTeacher(v as string)} />{submitted && !teacher && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field><Field label="教研组长" required><DeptStaffPicker staffList={deptMembers} value={groupLeader} onChange={(v) => setGroupLeader(v as string)} />{submitted && !groupLeader && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field></div>
          <div className="p-8 bg-white"><Field label="参与人员" required><Input value={participants} onChange={setParticipants} placeholder="多个姓名用逗号分隔" />{submitted && !participants && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field></div>
