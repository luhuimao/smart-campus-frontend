...
import { ChevronDown, Pen, Menu } from "lucide-react";
...
export function ClassTransferPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const handleSubmit = () => {
    setSubmitted(true);
    const base = [studentName, reason];
    if (changeType === "变更选科和班级") base.push(oldSubject, oldLang, oldClass, newSubject, newLang, newClass);
    else if (changeType === "仅变更选科") base.push(oldSubject, oldLang, newSubject, newLang);
    else base.push(oldClass, newClass);
    if (base.find((f) => !f)) window.scrollTo({ top: 0, behavior: "smooth" });
  };
  ...
  return (<div className="flex flex-col h-full overflow-hidden" style={{ color: "#1d1d1f" }}>
    ...
    <div className="flex-1 overflow-y-auto bg-[#f5f5f7] pb-24"><div className="max-w-5xl mx-auto mt-4 md:mt-10 px-3 md:px-6"><div className="rounded-2xl md:rounded-[28px] shadow-sm border border-gray-100 bg-white px-6 md:px-10 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
        <Field label="学生班级"><Input value={studentClass} onChange={setStudentClass} /></Field>
        ...
        <Field label="变更类型" required><div className="flex flex-wrap items-center gap-3 pt-1">{CHANGE_TYPES.map(({ label, color }) => (<label key={label} className="inline-flex items-center gap-2 cursor-pointer select-none"><span onClick={() => setChangeType(label)} className="shrink-0 flex items-center justify-center rounded-full border-2 transition-colors" style={{ width: 18, height: 18, borderColor: changeType === label ? "#10b981" : "#d1d5db", background: "white" }}>{changeType === label && (<span className="rounded-full" style={{ width: 10, height: 10, background: "#10b981", display: "block" }} />)}</span><span className="px-3 py-0.5 rounded text-xs font-semibold text-white transition-opacity" style={{ background: color, opacity: changeType === label ? 1 : 0.45 }}>{label}</span></label>))}</div></Field>
        <Field label="转科（班）原因" required><textarea rows={6} value={reason} onChange={(e) => setReason(e.target.value)} className="form-input resize-none" onFocus={(e) => Object.assign(e.currentTarget.style, focusStyle)} onBlur={(e) => Object.assign(e.currentTarget.style, blurStyle)} />{submitted && !reason && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>

        {changeType === "变更选科和班级" && (<>
          <Field label="原选科科目" required><Input value={oldSubject} onChange={setOldSubject} placeholder="请输入原选科科目" />{submitted && !oldSubject && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
          <Field label="原外语选科" required><Input value={oldLang} onChange={setOldLang} placeholder="请输入原外语选科" />{submitted && !oldLang && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
          <Field label="原班级" required><SelectField value={oldClass} onChange={setOldClass} options={["高一(1)班", "高一(2)班", "高二(1)班", "高三(1)班"]} />{submitted && !oldClass && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
          <Field label="新选科科目" required><SelectField value={newSubject} onChange={setNewSubject} options={["物理+化学+生物", "物理+化学+地理", "历史+政治+地理", "物理+历史+化学"]} />{submitted && !newSubject && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
          <Field label="新外语选科" required><SelectField value={newLang} onChange={setNewLang} options={["英语", "日语", "德语", "法语"]} />{submitted && !newLang && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
          <Field label="新班级" required><SelectField value={newClass} onChange={setNewClass} options={["高一(1)班", "高一(2)班", "高二(1)班", "高三(1)班"]} />{submitted && !newClass && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        </>)}
      ...
      {changeType !== "仅变更班级" && (<div className="mt-6"><Field label="家长签名" required><div className="flex items-center justify-center gap-2 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors" style={{ height: 100, color: "#6b7280" }}><Pen size={16} /><span className="text-sm">添加签名</span></div>{submitted && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field></div>)}
    </div></div></div>
