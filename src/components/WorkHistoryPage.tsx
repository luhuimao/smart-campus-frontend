...
import { JDY_CONFIG, TEACHER_TEACHING_EXPERIENCE_WIDGET_IDS, jdyCreate } from "@/lib/jdy-api";
...
export function WorkHistoryPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const [rows, setRows] = useState<ExpRow[]>([
    { id: 1, unit: "", startDate: "", endDate: "", position: "", subject: "" },
  ]);
  ...
  const addRow = useCallback(() => {
    setRows((prev) => [...prev, { id: nextId, unit: "", startDate: "", endDate: "", position: "", subject: "" }]);
    setNextId((n) => n + 1);
  }, [nextId]);
  ...
  const anyMissing = useMemo(
    () => rows.some((r) => !r.unit || !r.startDate || !r.position || !r.subject),
    [rows],
  );
  ...
  const H = TEACHER_TEACHING_EXPERIENCE_WIDGET_IDS;

  const buildData = () => {
    const subRecords = rows.map(row => ({
      [H["工作履历.编号"]]: { value: "" },
      [H["工作履历.任职单位"]]: { value: row.unit },
      [H["工作履历.开始日期"]]: { value: row.startDate },
      [H["工作履历.结束日期"]]: { value: row.endDate },
      [H["工作履历.任职岗位"]]: { value: row.position },
      [H["工作履历.所教学科"]]: { value: row.subject },
    }));

    const data: Record<string, { value: unknown }> = {
      [H.教师姓名]: { value: toMember(teacherName) },
      [H.身份证号码]: { value: idCard },
      [H.有无工作经历]: { value: hasExp === "yes" ? "有" : "暂无" },
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await jdyCreate({
        app_id: JDY_CONFIG.TEACHER_TEACHING_EXPERIENCE_INFO.app_id,
        entry_id: JDY_CONFIG.TEACHER_TEACHING_EXPERIENCE_INFO.entry_id,
        data: buildData(),
  ...
  const handleSaveDraft = () => {
    ...
    setDraftToast(true);
    localStorage.setItem("work-history-draft", JSON.stringify({
      teacherName, idCard, department, position, positionType, partyJob, phone, subject,
      hasExp, rows: rows.map(r => ({ id: r.id, unit: r.unit, startDate: r.startDate, endDate: r.endDate, position: r.position, subject: r.subject })),
    }));
  };
  ...
  const handleClearForm = () => {
    ...
    setRows([{ id: 1, unit: "", startDate: "", endDate: "", position: "", subject: "" }]);
    setNextId(2);
    setSubmitted(false);
    localStorage.removeItem("work-history-draft");
  };
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("work-history-draft");
      if (!raw) return;
      ...
      if (d.hasExp) setHasExp(d.hasExp);
      if (d.rows && d.rows.length > 0) {
        setRows(d.rows.map((r: ExpRow) => r));
        setNextId(Math.max(...d.rows.map((r: ExpRow) => r.id)) + 1);
      }
    } catch {}
  }, []);
  ...
  return (
    ...
    >
      <PageHeader
        centered
        breadcrumbs={[{ label: "教师基础档案" }, { label: "工作履历", active: true }]}
        ...
        <main className="max-w-6xl mx-auto pt-4 md:pt-6 px-3 md:px-6 pb-24">
          ...
          <div className="mb-8 text-center">
            <div className="inline-flex flex-col items-center">
              <h2 className="text-xl font-bold tracking-tight" style={{ color: "#111827" }}>工作履历</h2>
          ...
          <div className="rounded-[28px] overflow-hidden shadow-sm border border-gray-100 bg-white">
            ...
            <div className="p-8 bg-white">
              <Field label="有无工作经历" required>
                <div className="flex items-center gap-10 py-2">
                  {[{ v: "yes" as const, l: "有" }, { v: "no" as const, l: "暂无" }].map(({ v, l }) => (
                    <label key={v} className="flex items-center gap-2.5 cursor-pointer" onClick={() => setHasExp(v)}>
                      <div
                        className="w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all shrink-0 bg-white"
                        style={{ borderColor: hasExp === v ? teal : "#d1d5db" }}
                      >
                        {hasExp === v && <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: teal }} />}
                      </div>
              ...
              <>
                ...
                <div className="p-8 bg-white border-t border-gray-50">
                  <p className="text-base font-semibold mb-5">
                    <span style={{ color: "#ff4d4f", marginRight: 4 }}>*</span>工作履历
                  </p>

                  <div className="w-full border border-gray-100 rounded-xl overflow-hidden text-sm"
                    style={{ display: "grid", gridTemplateColumns: "48px 1.4fr 1fr 1fr 1fr 1fr 52px" }}>
                    {["", "任职单位", "开始日期", "结束日期", "任职岗位", "所教学科", ""].map((h, i) => (
                      <div key={i} className="px-3 py-3 bg-gray-50 text-sm font-semibold text-gray-600 border-b border-r border-gray-100 last:border-r-0">
                        {(i === 1 || i === 2 || i === 4 || i === 5) && <span style={{ color: "#ff4d4f", marginRight: 3 }}>*</span>}
                        {h}
                      </div>
                    ))}
                    ...
                    {rows.map((row, idx) => (
                      <>
                        ...
                        <div key={`po-${row.id}`} className="px-2 py-2 bg-white border-b border-r border-gray-100">
                          <input
                            type="text"
                            value={row.position}
                            onChange={(e) => updateRow(row.id, "position", e.target.value)}
                            className="table-input"
                            style={submitted && !row.position ? { borderColor: "#ff4d4f" } : undefined}
                            onFocus={(e) => Object.assign(e.currentTarget.style, focusStyle)}
                            onBlur={(e) => Object.assign(e.currentTarget.style, blurStyle)}
                          />
                        </div>
                        <div key={`su-${row.id}`} className="px-2 py-2 bg-white border-b border-r border-gray-100">
                          <CellSelect value={row.subject} onChange={(v) => updateRow(row.id, "subject", v)} options={["数学", "语文", "英语", "物理", "化学", "生物", "历史", "地理", "政治"]} />
                        </div>
                  ...
                  {submitted && hasExp === "yes" && anyMissing && (
                    <div className="mt-3 space-y-0.5">
                      {rows.some((r) => !r.unit) && <p className="text-xs" style={{ color: "#ff4d4f" }}>任职单位为必填项</p>}
                      {rows.some((r) => !r.startDate) && <p className="text-xs" style={{ color: "#ff4d4f" }}>开始日期为必填项</p>}
                      {rows.some((r) => !r.position) && <p className="text-xs" style={{ color: "#ff4d4f" }}>任职岗位为必填项</p>}
                      {rows.some((r) => !r.subject) && <p className="text-xs" style={{ color: "#ff4d4f" }}>所教学科为必填项</p>}
                    </div>
                  )}
