...
const BUILDING_OPTIONS = ["A栋", "B栋", "C栋", "D栋"];
const FLOOR_OPTIONS    = ["1楼", "2楼", "3楼", "4楼", "5楼", "6楼"];
...
const INITIAL_ROWS: CivilizedDormRow[] = [
...
const COLUMNS: ColDef<CivilizedDormRow>[] = [
  { key: "year",          label: "年份" },
...
const SORT_OPTIONS: { label: string; field: keyof CivilizedDormRow }[] = [
  { label: "提交时间", field: "submitTime" },
  { label: "更新时间", field: "updateTime" },
  { label: "年份",     field: "year" },
  { label: "周次",     field: "week" },
  { label: "楼栋",     field: "building" },
];
...
export function CivilizedDormPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const [previewRecord, setPreviewRecord] = useState<CivilizedDormRow | null>(null);
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);
  const [sortField, setSortField] = useState<keyof CivilizedDormRow>("submitTime");
  ...
  const [rows, setRows] = useState<CivilizedDormRow[]>(INITIAL_ROWS);
  const [nextId, setNextId] = useState(INITIAL_ROWS.length + 1);

  // Form state
  const [editRecord, setEditRecord] = useState<CivilizedDormRow | null>(null);
  ...
  useEffect(() => {
    ...
    setBuilding(editRecord.building);
    setFloor(editRecord.floor);
    setDormNo(editRecord.dormNo);
  ...
  useEffect(() => {
    ...
    try {
      const rawDraft = localStorage.getItem("civilized-dorm-draft");
      if (!rawDraft) return;
  ...
  const validate = (): boolean => {
    ...
    if (!building.trim())       errs.building = "请选择楼栋";
    if (!floor.trim())          errs.floor = "请选择楼层";
    if (!dormNo.trim())         errs.dormNo = "请输入宿舍号";
    ...
    try {
      ...
      if (isEditMode) {
        setRows(prev => prev.map(r => r.id === editRecord!.id
          ? { ...r, year, week, building, floor, dormNo, grade, className, headTeacher, gradeDirector, updateTime: ts }
          : r));
      } else {
        const newRow: CivilizedDormRow = {
          id: nextId,
          year, week, building, floor, dormNo, grade, className, headTeacher, gradeDirector,
      ...
      localStorage.removeItem("civilized-dorm-draft");
      if (isEditMode) setEditRecord(null);
      setShowForm(false);
      setDataMode(DATA_MODES[0]);
      setYear(""); setWeek(""); setBuilding(""); setFloor(""); setDormNo("");
      setGrade(""); setClassName(""); setHeadTeacher(""); setGradeDirector("");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSaveDraft = () => {
    ...
    localStorage.setItem("civilized-dorm-draft", JSON.stringify({
      year, week, building, floor, dormNo, grade, className, headTeacher, gradeDirector,
    }));
  };
  ...
  const doExport = (data: CivilizedDormRow[]) => {
    const headers = COLUMNS.map(c => c.label);
    const exportRows = data.map(r => COLUMNS.map(c => String(r[c.key as keyof CivilizedDormRow] ?? "")));
    const ws = XLSX.utils.aoa_to_sheet([headers, ...exportRows]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "文明宿舍");
    XLSX.writeFile(wb, `班主任所带文明宿舍_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ color: "#111827" }}>
      ...
      <div className="flex-1 overflow-y-auto bg-[#f5f5f7]">
        ...
        <div className="max-w-7xl mx-auto px-3 md:px-6 pt-4 md:pt-6">
            ...
            {dropdownOpen && (
              <div
                ...
                style={{
                  ...
                  animation: "civilizedDormDropdownIn 0.15s ease-out",
                }}
              >
          ...
          <style>{`@keyframes civilizedDormDropdownIn { from { opacity: 0; transform: translateY(-4px); } to { opacity: 1; transform: translateY(0); } }`}</style>
        </div>
        ...
        {(showForm || editRecord) ? (
          <main className="max-w-6xl mx-auto px-3 md:px-6 pb-24">
            ...
            <div className="rounded-[28px] shadow-sm border border-gray-100 bg-white">
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 bg-white">
                <Field label="楼栋" required error={errors.building}>
                  <SelectField value={building} onChange={v => { setBuilding(v); clearError("building"); }} options={BUILDING_OPTIONS} />
                </Field>
                <Field label="楼层" required error={errors.floor}>
                  <SelectField value={floor} onChange={v => { setFloor(v); clearError("floor"); }} options={FLOOR_OPTIONS} />
                </Field>
              </div>
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 bg-white">
                <Field label="宿舍号" required error={errors.dormNo}>
                  <TextInput value={dormNo} onChange={v => { setDormNo(v); clearError("dormNo"); }} placeholder="例：301" />
                </Field>
          ...
          <div className="max-w-7xl mx-auto px-3 md:px-6 pt-4 md:pt-6 pb-24">
            <div className="glass rounded-[32px] overflow-hidden flex flex-col shadow-sm" style={{ minHeight: 500 }}>
              ...
              <DataTable
                ...
                onRowClick={r => setPreviewRecord(r as CivilizedDormRow)}
              />
                ...
                <>
                  <div className="flex items-start justify-between px-6 py-5 border-b border-gray-100 shrink-0">
                    <div className="flex-1 min-w-0 pr-4">
                      ...
                      <h2 className="text-base font-bold text-gray-900 leading-snug">{r.dormNo || "—"} · {r.className || "—"}</h2>
                    </div>
                  ...
                  <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
                    <section>
                      ...
                      <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                        {[
                          { label: "楼栋", value: r.building }, { label: "楼层", value: r.floor },
                          { label: "宿舍号", value: r.dormNo }, { label: "级部", value: r.grade },
                          { label: "班级", value: r.className }, { label: "班主任", value: r.headTeacher },
                          { label: "级部主任", value: r.gradeDirector }, { label: "年份", value: r.year },
                          { label: "周次", value: r.week },
