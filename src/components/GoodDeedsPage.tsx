...
export function GoodDeedsPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const handleSubmit = () => { setSubmitted(true); if ([semester, fillDate, filler, grade, className, studentName, stuId, eventDate, eventDesc, classTeacher].find((f) => !f)) window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  return (<div className="flex flex-col h-full overflow-hidden" style={{ color: "#1d1d1f" }}>
    ...
    <div className="flex-1 overflow-y-auto bg-[#f5f5f7] pb-24"><div className="max-w-5xl mx-auto mt-4 md:mt-10 px-3 md:px-6"><div className="rounded-2xl md:rounded-[28px] shadow-sm border border-gray-100 bg-white px-6 md:px-10 py-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
        ...
        <Field label="填报时间" required><DateTimePicker value={fillDate} onChange={setFillDate} />{submitted && !fillDate && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        <Field label="填报人" required><MiniStaffPicker value={filler} onChange={setFiller} />{submitted && !filler && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        ...
        <Field label="事件发生时间" required><DateTimePicker value={eventDate} onChange={setEventDate} />{submitted && !eventDate && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        <Field label="事件地点"><SelectField value={eventPlace} onChange={setEventPlace} options={["校内","校外"]} />{eventPlace && <div className="mt-2"><textarea placeholder="请填写详细地址" rows={3} value={eventAddress} onChange={(e) => setEventAddress(e.target.value)} className="form-input resize-none" onFocus={(e) => Object.assign(e.currentTarget.style, focusStyle)} onBlur={(e) => Object.assign(e.currentTarget.style, blurStyle)} /></div>}</Field>
        <Field label="事件描述" required full><textarea placeholder="请输入事件描述" rows={4} value={eventDesc} onChange={(e) => setEventDesc(e.target.value)} className="form-input resize-none" onFocus={(e) => Object.assign(e.currentTarget.style, focusStyle)} onBlur={(e) => Object.assign(e.currentTarget.style, blurStyle)} />{submitted && !eventDesc && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        <Field label="班主任" required><MiniStaffPicker value={classTeacher} onChange={setClassTeacher} />{submitted && !classTeacher && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
