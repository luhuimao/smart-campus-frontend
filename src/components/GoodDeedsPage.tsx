...
import { useGoodDeeds, useTermInfo, useGradeInfo, useDepartmentMembers, type GoodDeedsRecord, type StudentInfoRecord } from "@/hooks/use-research-dashboard";
...
export function GoodDeedsPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const [previewRecord, setPreviewRecord] = useState<GoodDeedsRecord | null>(null);
  ...
  const perms = useFormPermissions(JDY_CONFIG.GOOD_DEEDS_INFO.entry_id);
              ...
              <div className="p-8 space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                  ...
                  <Field label="填报人" required>
                    <DeptStaffPicker staffList={deptMembers} value={filler} onChange={v => { setFiller(v as string); clearError("filler"); }} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                  <Field label="班主任" required>
                    <DeptStaffPicker staffList={deptMembers} value={classTeacher} onChange={v => { setClassTeacher(v as string); clearError("classTeacher"); }} />
            ...
            <div className="glass rounded-[32px] overflow-hidden flex flex-col shadow-sm" style={{ minHeight: 500 }}>
              ...
              {isPending ? (
                ...
                <DataTable columns={COLUMNS} rows={sorted} minWidth={1200}
                  ...
                  onRowClick={r => setPreviewRecord(r as GoodDeedsRecord)} />
              )}
        ...
        <div className="fixed top-0 right-0 h-full z-50 flex flex-col shadow-2xl" style={{ width: 440, maxWidth: "100vw", background: "#fff", transform: r ? "translateX(0)" : "translateX(100%)", transition: "transform 0.3s" }}>
          {r && (<>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6"><section><p className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3">基本信息</p><div className="grid grid-cols-2 gap-x-6 gap-y-4">
              {[{ label: "学期", value: r.学期 },{ label: "年级", value: r.年级 },{ label: "班级", value: r.班级名称 },{ label: "学生姓名", value: r.学生姓名 },{ label: "事件时间", value: r.事件发生时间?.slice(0,10) },{ label: "事件地点", value: r.事件地点 },{ label: "事件描述", value: r.事件描述 },{ label: "班主任", value: r.班主任 },{ label: "填报人", value: r.提交人 }].map(({ label, value }) => (<div key={label}><p className="text-sm text-gray-400 mb-0.5">{label}</p><p className="text-base font-medium text-gray-800">{value||"—"}</p></div>))}
            </div></section></div>
