...
type ProjectRow = { id: number; name: string; level: string; status: string; image: string | null };
...
export function ProjectPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const [rows, setRows] = useState<ProjectRow[]>([{ id: 1, name: "", level: "", status: "", image: null }]);
  const [nextId, setNextId] = useState(2);

  const updateRow = useCallback((id: number, field: string, value: string | null) => setRows((p) => p.map((r) => (r.id === id ? { ...r, [field]: value } : r))), []);
  const addRow = useCallback(() => { setRows((p) => [...p, { id: nextId, name: "", level: "", status: "", image: null }]); setNextId((n) => n + 1); }, [nextId]);
  const removeRow = useCallback((id: number) => setRows((p) => (p.length <= 1 ? p : p.filter((r) => r.id !== id))), []);
  const anyMissing = useMemo(() => rows.some((r) => !r.name || !r.level || !r.status || !r.image), [rows]);
    ...
    <div className="flex-1 overflow-y-auto bg-[#f5f5f7] pb-24"><main className="max-w-6xl mx-auto mt-4 md:mt-10 px-3 md:px-6">
      ...
      <div className="rounded-[28px] overflow-hidden shadow-sm border border-gray-100 bg-white">
        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 bg-white">
          <Field label="教师姓名" required><StaffPicker value={teacherName} onChange={setTeacherName} onSelectRecord={handleSelectStaff} />{submitted && !teacherName && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
          <Field label="身份证号码" required><Input value={idCard} onChange={setIdCard} />{submitted && !idCard && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>此项为必填项</p>}</Field>
        </div>
        <div className="p-8 bg-white"><Field label="有无课题" required><div className="flex items-center gap-10 py-2">{[{ v: "yes" as const, l: "有" }, { v: "no" as const, l: "暂无" }].map(({ v, l }) => (<label key={v} className="flex items-center gap-2.5 cursor-pointer" onClick={() => setHasProject(v)}><div className="w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all shrink-0 bg-white" style={{ borderColor: hasProject === v ? teal : "#d1d5db" }}>{hasProject === v && <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: teal }} />}</div><span className="text-base font-medium text-gray-700">{l}</span></label>))}</div></Field></div>
        {hasProject === "yes" && (<>
          ...
          <div className="p-8 bg-white border-t border-gray-50">
            <p className="text-base font-semibold mb-5"><span style={{ color: "#ff4d4f", marginRight: 4 }}>*</span>课题</p>
            <div className="w-full border border-gray-100 rounded-xl overflow-hidden text-sm" style={{ display: "grid", gridTemplateColumns: "48px 2fr 1fr 1.2fr 1fr 52px" }}>
              {["", "课题名称", "级别", "立项/结项", "图片证明材料", ""].map((h, i) => (<div key={i} className="px-3 py-3 bg-gray-50 text-sm font-semibold text-gray-600 border-b border-r border-gray-100 last:border-r-0">{i > 0 && i !== 4 && i !== 5 && <span style={{ color: "#ff4d4f", marginRight: 3 }}>*</span>}{h}</div>))}
              {rows.map((row, idx) => (<>
                ...
                <div key={`lv-${row.id}`} className="px-2 py-2 bg-white border-b border-r border-gray-100"><CellSelect value={row.level} onChange={(v) => updateRow(row.id, "level", v)} options={["国家级", "省级", "市级", "区县级", "校级"]} /></div>
                <div key={`st-${row.id}`} className="px-2 py-2 bg-white border-b border-r border-gray-100"><CellSelect value={row.status} onChange={(v) => updateRow(row.id, "status", v)} options={["立项", "结项"]} /></div>
            ...
            {submitted && hasProject === "yes" && anyMissing && (<div className="mt-3 space-y-0.5">{rows.some((r) => !r.name) && <p className="text-xs" style={{ color: "#ff4d4f" }}>课题名称为必填项</p>}{rows.some((r) => !r.level) && <p className="text-xs" style={{ color: "#ff4d4f" }}>级别为必填项</p>}{rows.some((r) => !r.status) && <p className="text-xs" style={{ color: "#ff4d4f" }}>立项/结项为必填项</p>}{rows.some((r) => !r.image) && <p className="text-xs" style={{ color: "#ff4d4f" }}>图片证明材料为必填项</p>}</div>)}
