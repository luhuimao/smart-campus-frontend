...
export function CivilizedDormFormModal({ onClose }: Props) {
  const [maximized, setMaximized] = useState(false);
  const [form, setForm] = useState<FormState>({
    year: "", week: "", building: "", floor: "", dormNo: "",
    grade: "", className: "", headTeacher: "", gradeDirector: "",
  });
        ...
        <div className="overflow-y-auto px-6 py-6 flex-1">
          <div className="grid grid-cols-2 gap-x-6 gap-y-5">
            ...
            <Field label="楼栋" required>
              <SelectInput options={BUILDING_OPTIONS} value={form.building} onChange={set("building")} />
            </Field>

            <Field label="楼层" required>
              <SelectInput options={FLOOR_OPTIONS} value={form.floor} onChange={set("floor")} />
            </Field>

            <Field label="宿舍号" required>
              <TextInput placeholder="例：301" value={form.dormNo} onChange={set("dormNo")} />
            </Field>

            <Field label="级部" required>
              <SelectInput options={GRADE_OPTIONS} value={form.grade} onChange={set("grade")} />
            </Field>
