...
function CellInput({ placeholder = "", center = false }: { placeholder?: string; center?: boolean }) {
  return (
    <input
      type="text"
      placeholder={placeholder}
      className={`w-full bg-white border border-gray-200 rounded-lg px-2 py-1 text-xs outline-none transition-all placeholder:text-gray-300 ${center ? "text-center" : ""}`}
      onFocus={(e) => Object.assign(e.currentTarget.style, focusStyle)}
      onBlur={(e) => Object.assign(e.currentTarget.style, blurStyle)}
    />
  );
...
function CellDate({ placeholder }: { placeholder?: string }) {
  return (
    <div className="w-full h-8 border border-gray-200 rounded-lg flex items-center justify-between px-2.5 bg-gray-50 text-gray-400 cursor-pointer hover:bg-gray-100 transition-colors">
      <span className="text-[10px]">{placeholder ?? "请选择"}</span>
              ...
              <>
                ...
                <div className="p-8 bg-white border-t border-gray-50">
                  ...
                  <div className="w-full border border-gray-100 rounded-xl overflow-hidden text-sm">
                    {/* Header */}
                    <div className="grid bg-gray-50 border-b border-gray-100" style={{ gridTemplateColumns: "48px 1.1fr 1.2fr 1fr 1.2fr 0.7fr" }}>
                      {[
                        { label: "", req: false },
                        { label: "兼职职务", req: true },
                        { label: "任职单位", req: true },
                        { label: "兼职开始日期", req: true },
                        { label: "兼职结束日期", req: true },
                        { label: "年限", req: true },
                      ].map((h, i) => (
                    ...
                    {/* Rows */}
                    {rows.map((row, index) => (
                      <div key={row.id} className="grid border-b border-gray-100 last:border-0" style={{ gridTemplateColumns: "48px 1.1fr 1.2fr 1fr 1.2fr 0.7fr" }}>
                        ...
                        <div className="px-2 py-2 border-r border-gray-100">
                          <CellSelect options={["顾问", "客座教授", "兼职讲师"]} />
                        </div>
                        {/* 任职单位 */}
                        <div className="px-2 py-2 border-r border-gray-100">
                          <CellInput />
                        ...
                        {/* 兼职结束日期 */}
                        <div className="px-2 py-2 border-r border-gray-100">
                          <CellDate placeholder="已结束的兼职选择填写" />
                        </div>
                        {/* 年限 */}
