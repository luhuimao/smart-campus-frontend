...
import { useHeartToHeartTalk, useCourses, useStaffDirectory, useDepartmentMembers, type HeartToHeartTalkRecord, type StudentInfoRecord } from "@/hooks/use-research-dashboard";
...
const H = STUDENT_HEART_TO_HEART_TALK_WIDGET_IDS;

const COLUMNS: ColDef<HeartToHeartTalkRecord>[] = [
  { key: "谈心教师",     label: "教师" },
  { key: "班级名称",     label: "班级" },
  { key: "学生姓名",     label: "学生" },
  { key: "谈心教师学科", label: "学科" },
...
const SORT_OPTIONS: { label: string; field: keyof HeartToHeartTalkRecord }[] = [
  { label: "提交时间", field: "提交时间" },
  { label: "谈心谈话时间", field: "谈心谈话时间" },
  { label: "班级名称", field: "班级名称" },
  { label: "谈心教师", field: "谈心教师" },
];
...
export function TalkRecordPage({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  // Form state
  const [editRecord, setEditRecord] = useState<HeartToHeartTalkRecord | null>(null);
  ...
  const existingPhotoUrls = useMemo(() => isEditMode ? editRecord!.沟通照片.map(f => f.url) : [], [editRecord, isEditMode]);
  ...
  useEffect(() => {
    ...
    setTeacher(editRecord.谈心教师);
    setClassName(editRecord.班级名称);
    setStudentName(editRecord.学生姓名);
    setIdCard(editRecord.学生身份证);
    setStuId(editRecord.学生学号);
    setTeacherSubject(editRecord.谈心教师学科);
    setTalkTime(editRecord.谈心谈话时间 ? editRecord.谈心谈话时间.slice(0, 16) : "");
    setSelectedTopics(editRecord.谈话内容 ? editRecord.谈话内容.split("、") : []);
    setContentRecord(editRecord.谈心谈话内容记录);
    setAdvice(editRecord.教师指导建议);
    setPhotos([]);
  }, [editRecord]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (d.teacherSubject) setTeacherSubject(d.teacherSubject);
      if (d.talkTime) setTalkTime(d.talkTime);
  ...
  const validate = (): boolean => {
    ...
    if (!teacher.trim())          errs.teacher = "请选择谈心教师";
    if (!studentName.trim())      errs.studentName = "请选择学生";
    if (!teacherSubject.trim())   errs.teacherSubject = "请选择学科";
    if (!talkTime.trim())         errs.talkTime = "请选择谈话时间";
    if (selectedTopics.length === 0) errs.topics = "请至少选择一项谈话内容";
    if (!contentRecord.trim())    errs.contentRecord = "请输入谈话内容记录";
    if (!advice.trim())           errs.advice = "请输入教师指导建议";
    setErrors(errs);
  ...
  const buildData = (photoKeys?: string[]) => {
    ...
    const data: Record<string, { value: unknown }> = {
      [H.谈心教师]: { value: toMemberUsername(teacher) },
      [H.班级名称]: { value: className },
      [H.学生姓名]: { value: studentName },
      [H.学生身份证]: { value: idCard },
      [H.学生学号]: { value: stuId },
      [H.谈心教师学科]: { value: teacherSubject },
      [H.谈心谈话时间]: { value: talkTime },
      [H.谈话内容]: { value: selectedTopics.join("、") },
      [H.谈心谈话内容记录]: { value: contentRecord },
      [H.教师指导建议]: { value: advice },
      [H.提交人]: { value: isEditMode ? editRecord!.提交人 : (currentUser?.name ?? "") },
      [H.提交时间]: { value: isEditMode ? editRecord!.提交时间 : now },
    };
    if (photoKeys !== undefined) data[H.沟通照片] = { value: photoKeys };
    return data;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (photos.length > 0) {
        ...
        const result = await jdyUploadFiles(photos, JDY_CONFIG.STUDENT_HEART_TO_HEART_TALK.app_id, JDY_CONFIG.STUDENT_HEART_TO_HEART_TALK.entry_id, transaction_id);
        photoKeys = isEditMode ? [...editRecord!.沟通照片.map(f => f.key ?? f.name), ...result.keys] : result.keys;
      }
  ...
  const clearFormFields = () => {
    setTeacher(""); setStudentName(""); setClassName(""); setStuId(""); setIdCard("");
    setTeacherSubject(""); setTalkTime(""); setSelectedTopics([]); setContentRecord("");
    setAdvice(""); setPhotos([]);
    setErrors({});
  };

  const handleClearForm = () => { clearFormFields(); localStorage.removeItem("talk-record-draft"); };

  const handleSaveDraft = () => {
    if (!teacher.trim() && !studentName.trim()) return;
    localStorage.setItem("talk-record-draft", JSON.stringify({
      teacher, studentName, className, stuId, idCard, teacherSubject, talkTime,
      selectedTopics, contentRecord, advice,
    }));
  };
  ...
  const handleDeleteSelected = async (opt: string) => {
    ...
    try {
      for (const r of targetData) { await jdyDelete({ app_id: JDY_CONFIG.STUDENT_HEART_TO_HEART_TALK.app_id, entry_id: JDY_CONFIG.STUDENT_HEART_TO_HEART_TALK.entry_id, data_id: r._id }); }
      queryClient.invalidateQueries({ queryKey: ["heart-to-heart-talk", "list"] });
    } catch (err) { alert(err instanceof Error ? err.message : "删除失败，请重试"); }
  };

  const doExport = (data: HeartToHeartTalkRecord[]) => {
    const headers = COLUMNS.map(c => c.label);
    const rows = data.map(r => COLUMNS.map(c => String(r[c.key as keyof HeartToHeartTalkRecord] ?? "")));
    const ws = XLSX.utils.aoa_to_sheet([headers, ...rows]);
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "谈心谈话记录");
    XLSX.writeFile(wb, `谈心谈话记录_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  const filtered = tableData.filter(r =>
    r.学生姓名.includes(search) || r.谈心教师.includes(search) || r.班级名称.includes(search)
  );
          ...
          <main className="max-w-6xl mx-auto px-3 md:px-6 pb-24">
            <div className="mb-8 text-center">
              <div className="inline-flex flex-col items-center">
                <h2 className="text-xl font-bold tracking-tight" style={{ color: "#111827" }}>{editRecord ? "编辑谈心谈话" : "一生一案谈心谈话记录表"}</h2>
            ...
            <div className="rounded-[28px] shadow-sm border border-gray-100 bg-white">
              <div className="p-8 space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                  ...
                  <Field label="班级名称">
                    <TextInput value={className} onChange={setClassName} placeholder="自动填充" />
                  </Field>
                  <Field label="学生学号">
                    <TextInput value={stuId} onChange={setStuId} placeholder="自动填充" />
                  </Field>
                  <Field label="学生身份证">
                    <TextInput value={idCard} onChange={setIdCard} placeholder="自动填充" />
                  </Field>
                  <Field label="谈心教师学科" required>
                    <SelectField value={teacherSubject} onChange={v => { setTeacherSubject(v); clearError("teacherSubject"); }} options={courseOptions.length > 0 ? courseOptions : ["语文","数学","英语","物理","化学","生物","历史","地理","政治"]} />
                    {errors.teacherSubject && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>{errors.teacherSubject}</p>}
                  </Field>
                  <Field label="谈心谈话时间" required>
                    <input type="datetime-local" value={talkTime} onChange={e => { setTalkTime(e.target.value); clearError("talkTime"); }} className="form-input"
                      style={{ color: talkTime ? "#1d1d1f" : "#9ca3af" }}
                      onFocus={e => Object.assign(e.currentTarget.style, focusStyle)} onBlur={e => Object.assign(e.currentTarget.style, blurStyle)} />
                    {errors.talkTime && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>{errors.talkTime}</p>}
                  </Field>
                </div>
                ...
                <Field label="谈话内容" required>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-y-3 pt-1">
                    {TALK_TOPICS.map(t => (
                      <label key={t} className="flex items-center gap-2 cursor-pointer select-none" onClick={() => { toggleTopic(t); clearError("topics"); }}>
                        <span className="shrink-0 flex items-center justify-center rounded-full border transition-colors" style={{ width: 18, height: 18, borderColor: selectedTopics.includes(t) ? teal : "#d1d5db", background: "white" }}>
                          {selectedTopics.includes(t) && <span className="rounded-full" style={{ width: 10, height: 10, background: teal, display: "block" }} />}
                        </span>
                        <span className="text-sm" style={{ color: "#374151" }}>{t}</span>
                      </label>
                    ))}
                  </div>
                  {errors.topics && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>{errors.topics}</p>}
                </Field>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                  <Field label="谈心谈话内容记录" required>
                    <textarea rows={6} value={contentRecord} onChange={e => { setContentRecord(e.target.value); clearError("contentRecord"); }}
                      className="form-input resize-none" placeholder="请输入谈话内容记录"
                      onFocus={e => Object.assign(e.currentTarget.style, focusStyle)} onBlur={e => Object.assign(e.currentTarget.style, blurStyle)} />
                    {errors.contentRecord && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>{errors.contentRecord}</p>}
                  </Field>
                  <Field label="教师指导建议" required>
                    <textarea rows={6} value={advice} onChange={e => { setAdvice(e.target.value); clearError("advice"); }}
                      className="form-input resize-none" placeholder="请输入教师指导建议"
                      onFocus={e => Object.assign(e.currentTarget.style, focusStyle)} onBlur={e => Object.assign(e.currentTarget.style, blurStyle)} />
                    {errors.advice && <p className="text-xs mt-1.5" style={{ color: "#ff4d4f" }}>{errors.advice}</p>}
                  </Field>
                </div>
